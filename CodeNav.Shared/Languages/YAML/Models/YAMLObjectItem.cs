...
namespace CodeNav.Shared.Languages.YAML.Models
{
    ...
    {
        public YAMLObjectItem(YamlScalarNode keyNode, YamlNode valueNode, string name = null) : base()
        {
            Name = name != null ? name : keyNode.Value;
            FullName = Name;
            Id = Name;
            Tooltip = Name;
        ...
        public override string ToString() => $"{Tab.Repeat(Depth)}yamlProperty(name={Name},depth={Depth},startLine={StartLine},endLine={EndLine}))\n{string.Join(NewLine, Members.Select(member => member.ToString()))}";
